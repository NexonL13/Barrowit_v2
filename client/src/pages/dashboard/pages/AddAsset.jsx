...
const AddAsset = () => {
  ...
  const validationSchema = Yup.object({
    ...
    description: Yup.string().min(6, "Must be at least 6 characters").required(),
    stock: Yup.number().min(1, 'Must be at least 1 stock').required('Stock is a required field'),
    category: Yup.string().required("Category is a required field").notOneOf([""], "Select a valid category"),
  ...
  const onSubmit = async (values) => {
    ...
    await axios.post("http://localhost:3000/asset/upload", formData).then((res)=> {
      if(res.data.error) {
        setExist(true)
      }else {
      navigate('/')
  ...
  <div className="lg:grid lg:min-h-screen lg:grid-cols-none">
    ...
    >
      <div className="max-w-xl lg:max-w-3xl">
        ...
        <div className={`${
          exist ? "visible" : "hidden"} col-span-12`}>
        <WarningError warningTitle="Assest Already Exist" warningText="This assets is already added. Add another asset."/>
        </div>
        ...
        <Field name="image">
          {() => (
            ...
            <input type="file"
                      ...
                      <label htmlFor="image" className="z-20 flex flex-col-reverse items-center justify-center w-full h-full cursor-pointer">
        <p className="z-10 text-xs font-light text-center text-gray-500">{fileName ? `${fileName}` : `Drag & Drop your files here`}</p>
        <svg className="z-10 w-8 h-8 text-teal-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
